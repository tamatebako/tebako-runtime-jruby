# tamatebako/jruby — the JRuby runtime feedstock
...
(tamatebako/openjdk — `DEPENDS java >= 21`, spec 33's `on_runtime` form).
...
- Spec: docs/spec/33 (runtime-on-runtime) in tamatebako/tebako; the jvm-mode
  machinery mirrors tamatebako/truffleruby's `flavors.jvm` (its /03 PR).
